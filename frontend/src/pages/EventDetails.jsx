...
const EventDetails = () => {
  ...
  const handleRegistration = async () => {
    ...
    if (event.eventType === "merchandise" && !registrationData.selectedItem) {
      setRegistrationResult({ success: false, message: "Please select an item to purchase." });
      return;
    }
    ...
    try {
      ...
      if (result && !result.error && (result.msg || result.ticket || result.success)) {
        ...
        setRegistrationResult({
          success: false,
          message: result?.error || result?.msg || "Registration failed. Please try again.",
        });
      }
    } catch (error) {
      setRegistrationResult({
  ...
  const getBlockingMessage = () => {
    ...
    if (blocking.registrationLimitExhausted) return "Registration limit has been reached";
  ...
  const availableItems = event.merchandise?.items?.filter((i) => i.stock > 0) || [];
  ...
  return (
    <PageContainer>
      <div className="event-details">
              ...
              <div className="event-details__meta">
                ...
                {event.eventType && (
                  <Badge variant="secondary" size="large">
                    {event.eventType === "merchandise" ? "🛍️ Merchandise" : "🎟️ Normal Event"}
                  </Badge>
                )}
        ...
        <div className="event-details__content">
          <div className="event-details__main">
            ...
            <section className="event-section">
              ...
              <div className="event-details__info-grid">
                ...
                <div className="info-item">
                  ...
                  <div className="info-value">{formatDate(event.eventEndDate)}</div>
                </div>

                <div className="info-item">
                  <div className="info-label">⏰ Start Time</div>
                  <div className="info-value">{formatTime(event.eventStartDate)}</div>
                </div>

                <div className="info-item">
                  <div className="info-label">⏰ End Time</div>
                  <div className="info-value">{formatTime(event.eventEndDate)}</div>
                </div>
                ...
                <div className="info-item">
                  ...
                  <div className="info-value">
                    {event.registrationCount || 0} / {event.registrationLimit || "Unlimited"} registered
                  </div>
                </div>
                ...
                {event.eventTags?.length > 0 && (
                  <div className="info-item" style={{ gridColumn: "1 / -1" }}>
                    ...
                    <div className="info-value" style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            ...
            {/* Merchandise Items */}
            {event.eventType === "merchandise" && event.merchandise?.items?.length > 0 && (
              <section className="event-section">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "12px" }}>
                  {event.merchandise.items.map((item, idx) => (
                    <div key={idx} style={{
                      ...
                      {item.size && <p style={{ margin: "0 0 2px", color: "#aaa", fontSize: "0.85rem" }}>Size: {item.size}</p>}
                      {item.color && <p style={{ margin: "0 0 2px", color: "#aaa", fontSize: "0.85rem" }}>Color: {item.color}</p>}
                      {item.variant && <p style={{ margin: "0 0 2px", color: "#aaa", fontSize: "0.85rem" }}>Variant: {item.variant}</p>}
                      <p style={{ margin: "4px 0 0", color: item.stock > 0 ? "#22c55e" : "#ef4444", fontSize: "0.85rem" }}>
                ...
                <div className="registration-available">
                  ...
                  {event.registrationLimit > 0 && (
                    <div className="slots-remaining">
                      {event.registrationLimit - (event.registrationCount || 0)} spots remaining
                    </div>
                  )}
                  ...
                  >
                    {event.eventType === "merchandise" ? "Purchase Now" : "Register Now"}
                  </Button>
                </div>
              ) : (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "8px" }}>
                  <a
                    ...
                    style={{
                      ...
                      color: "#a5b4fc", textDecoration: "none", fontSize: "0.9rem", fontWeight: 500,
                  ...
                  <a
                    ...
                    style={{
                      ...
                      color: "#86efac", textDecoration: "none", fontSize: "0.9rem", fontWeight: 500,
                    }}
                  >
                    🟢 Google Calendar
                  </a>
                  <a
                    ...
                    style={{
                      ...
                      color: "#93c5fd", textDecoration: "none", fontSize: "0.9rem", fontWeight: 500,
                    }}
                  >
                    🔵 Microsoft Outlook
                  </a>
      ...
      <Modal
        ...
        onClose={() => { setShowRegistrationModal(false); setRegistrationResult(null); }}
        title={event.eventType === "merchandise" ? "Complete Your Purchase" : "Complete Your Registration"}
        size="medium"
        footer={
          registrationResult?.success ? null : (
            <>
              <Button
                variant="secondary"
                onClick={() => { setShowRegistrationModal(false); setRegistrationResult(null); }}
                disabled={registering}
              >
                Cancel
              </Button>
        ...
        <div className="registration-form">
          ...
          {registrationResult && (
            <div style={{
              ...
              borderRadius: "8px",
              marginBottom: "16px",
              background: registrationResult.success ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)",
          ...
          {event.eventType === "merchandise" && !registrationResult?.success && (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {availableItems.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        onClick={() => setRegistrationData((prev) => ({ ...prev, selectedItem: item, quantity: 1 }))}
                        style={{
                          padding: "12px",
                      ...
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <div>
                            ...
                            {item.size && <span style={{ marginLeft: "8px", color: "#aaa" }}>Size: {item.size}</span>}
                            {item.color && <span style={{ marginLeft: "8px", color: "#aaa" }}>Color: {item.color}</span>}
                            {item.variant && <span style={{ marginLeft: "8px", color: "#aaa" }}>{item.variant}</span>}
                          </div>
                          <span style={{ color: "#22c55e", fontSize: "0.85rem", whiteSpace: "nowrap" }}>
                            {item.stock} in stock
                          </span>
                        </div>
                        ...
                        {isSelected && (
                          ...
                          >
                            <span style={{ fontSize: "0.85rem", color: "#aaa" }}>Quantity:</span>
                            <button
                              type="button"
                              onClick={() => setRegistrationData((prev) => ({ ...prev, quantity: Math.max(1, prev.quantity - 1) }))}
                              disabled={registrationData.quantity <= 1}
                              style={{
                                width: "28px", height: "28px", borderRadius: "6px",
                                border: "1px solid #6366f1", background: "transparent",
                                color: "#6366f1", cursor: "pointer", fontSize: "1rem",
                                display: "flex", alignItems: "center", justifyContent: "center",
                                opacity: registrationData.quantity <= 1 ? 0.4 : 1,
                              }}
                            >−</button>
                            <span style={{
                            ...
                            <button
                              type="button"
                              onClick={() => setRegistrationData((prev) => ({ ...prev, quantity: Math.min(maxQty, prev.quantity + 1) }))}
                              ...
                              style={{
                                ...
                                color: "#6366f1", cursor: "pointer", fontSize: "1rem",
                                display: "flex", alignItems: "center", justifyContent: "center",
                                opacity: registrationData.quantity >= maxQty ? 0.4 : 1,
                              }}
                            >+</button>
          ...
          {/* Merchandise summary */}
          {event.eventType === "merchandise" && registrationData.selectedItem && !registrationResult?.success && (
            <div className="registration-summary" style={{ marginTop: "16px" }}>
              ...
              <div className="summary-row">
                <span>Quantity:</span>
                <span style={{ fontWeight: 700, color: "#6366f1" }}>× {registrationData.quantity}</span>
              </div>
            </div>
          )}
        </div>
